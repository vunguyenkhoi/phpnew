...
@section('main-content')
    ...
    <div class="card">
        ...
        <!-- end card header -->
        <div class="card-body">
            <form name="frmEdit" action="{{ route('backend.shop_setting.update', ['id' => $ShopSetting->id]) }}"
                method="POST">
                @csrf @method('put')
                <div class="row gy-2">
                    <div class="col-6">
                        <div>
                            ...
                            <input type="text" name="group" class="form-control" id="group"
                                value="{{ $ShopSetting->group }}" placeholder="Nhập tên nhóm">
                        </div>
                    </div>
                    <div class="col-6">
                        <div>
                            ...
                            <input type="text" name="key" class="form-control" id="key"
                                value="{{ $ShopSetting->key }}" placeholder="Nhập vào tên khoá">
                        </div>
                    </div>
                    <div class="col-12">
                        <div>
                            ...
                            <input type="text" name="value" class="form-control" id="value"
                                value="{{ $ShopSetting->value }}" placeholder="Nhập vào giá trị">
                        </div>
                    </div>
                    <div class="col-12">
                        <label for="description" class="form-label ">Diễn giải</label>
                        <textarea name="description" class="form-control " id="description" placeholder="Nhập vào Diễn giải">{{ $ShopSetting->description }}</textarea>
                    </div>
                </div>
                <div class="mt-3">
